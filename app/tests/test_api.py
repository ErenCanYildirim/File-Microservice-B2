...
class TestFixedAPI:
    ...
    def test_upload_text_file(self):
        test_content = self.generate_unique_content(
            "Hello, World! This is a test file."
        )
        files = {"file": ("test.txt", BytesIO(test_content), "text/plain")}
        data = {"uploaded_by": "pytest_test_service"}
    ...
    def test_upload_image_file(self):
        ...
        data = {"uploaded_by": "pytest_image_service"}
    ...
    def test_upload_pdf_file(self):
        ...
        files = {
            "file": (
                f"test_{uuid.uuid4().hex[:8]}.pdf",
                BytesIO(pdf_content),
                "application/pdf",
            )
        }
        data = {"uploaded_by": "pytest_pdf_service"}
    ...
    def test_get_file_info(self):
        ...
        files = {"file": ("get_info_test.txt", BytesIO(test_content), "text/plain")}
        data = {"uploaded_by": "pytest_get_info_service"}
    ...
    def test_download_url_for_uploaded_file(self):
        ...
        data = {"uploaded_by": "pytest_download_service"}
    ...
    def test_concurrent_uploads(self):
        ...
        def upload_file(file_number):
            content = self.generate_unique_content(
                f"Concurrent test file {file_number}"
            )
            files = {
                "file": (
                    f"concurrent_{file_number}_{uuid.uuid4().hex[:8]}.txt",
                    BytesIO(content),
                    "text/plain",
                )
            }
            data = {"uploaded_by": f"concurrent_test_{file_number}"}
    ...
    def test_stress_upload_many_small_files(self):
        ...
        for i in range(10):
            ...
            files = {
                "file": (
                    f"stress_{i}_{uuid.uuid4().hex[:8]}.txt",
                    BytesIO(content),
                    "text/plain",
                )
            }
            data = {"uploaded_by": f"stress_test_{i}"}
        ...
        ), f"Only {successful_uploads}/10 uploads succeeded"

    def test_database_operations_via_api(self):
        content = self.generate_unique_content("CRUD test file content")
        files = {
            "file": (
                f"crud_test_{uuid.uuid4().hex[:8]}.txt",
                BytesIO(content),
                "text/plain",
            )
        }
        data = {"uploaded_by": "crud_test_service"}
        ...
        ), f"File should be deleted but got: {response.status_code}"

    def test_file_deduplication_feature(self):
        content = b"Deduplication test content - exact same bytes"

        files1 = {"file": ("dedup_test1.txt", BytesIO(content), "text/plain")}
        response1 = requests.post(f"{BASE_URL}/upload", files=files1)

        files2 = {"file": ("dedup_test2.txt", BytesIO(content), "text/plain")}
...
if __name__ == "__main__":
    print("Testing Live File Upload Service")
    print("=" * 70)
